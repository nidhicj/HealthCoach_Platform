...
echo "  Pure DB inspection — no LLM calls."
echo ""
echo "  What this script checks:"
echo "  1. Style snippets: how many were captured? From which sessions?"
echo "  2. LLM call breakdown: tokens and latency per use_case"
echo "  3. Snippet injection: was snippet_count > 0 in latest MOM per client?"
echo "     (proves the style flywheel is injecting learned voice into drafts)"
echo "  4. Brief token progression: did input_tokens GROW across sessions?"
echo "     (proves the context farm is accumulating prior session context)"
echo ""
echo "  GREEN FLAGS  ✓  snippets ≥ 5 total"
echo "               ✓  snippet_count ≥ 1 in Ravi and Sunita latest MOMs"
echo "               ✓  Ravi brief tokens: S1 < S2 < S3 < S4 < S5"
echo "               ✓  Sunita brief tokens: S1 < ... < S8"
echo ""
echo "  RED FLAGS    ✗  snippet_count = 0 in MOMs → flywheel broken"
echo "               ✗  flat token counts → context not injected"
echo "               ✗  0 style snippets → HC edits not triggering capture"
echo "======================================================="
