...
| HC's`users.first_name`/`users.last_name` are null (Unit_006 profile setup not completed) and HC opens `/settings/leadgen` | `POST /api/leadgen/config/init` returns a structured "profile incomplete" response (not a 500 or raw constraint error). Frontend redirects HC to `/settings/profile` to complete their name, then back to leadgen setup.                                                                                                                              |
...
- **Unit_006 PHASE-01 as a prerequisite**: this spec now depends on `Unit_006_PlatformFoundations` PHASE-01 (HC Settings & Profile) shipping `users.first_name`/`users.last_name` via `/api/settings/profile`, before Unit_003 Stage 1 (leadgen setup) can be implemented. Unit_006 PHASE-01 as currently spec'd does *not* include these fields (scope is `business_name` only) — it needs to be extended.
  - Owner: SoJo — by: before Unit_003 Phase 1 implementation begins; sequencing against Unit_006's own roadmap needs to be decided.
