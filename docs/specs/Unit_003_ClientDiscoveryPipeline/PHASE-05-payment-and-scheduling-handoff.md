...
- `backend/src/api/clients.py` — the invite-token creation mechanics (`os.urandom(32).hex()`, sha256 hash, URL construction from `frontend_url`) Task 4 mirrors. Its "invalidate existing unused tokens first" step is deliberately NOT mirrored (see Task 4 — a raw token can't be recovered from its hash, so there's nothing to re-send even if an old one were invalidated; Task 4 mints fresh every time instead).
...
`POST /api/hc/payment-account/connect` — body `{key_id: str, key_secret: str, webhook_secret: str}` (all required, non-blank). Calls `razorpay_client.verify_credentials()` (Task 1) with the pasted `key_id`/`key_secret`. On success: upsert the HC's `hc_payment_accounts` row (`credentials` = all three fields, `connected_at = now()`), return `{connected: true}`. On failure: structured 422 (not 500) — `{"error": "invalid_credentials", "message": "Could not verify these Razorpay credentials — check they're correct and in test mode."}`. Reconnecting (calling this a second time) overwrites the stored credentials and updates `updated_at` — no separate "disconnect" flow needed at this scope.

Register `payment_accounts_router` in `backend/src/main.py`.

Tests: connect success (mock `verify_credentials` → `True`, row created, `connected_at` set), connect failure (mock → `False`, structured 422, no row / row not marked connected), reconnect overwrites cleanly, `GET` before and after connect, cross-tenant isolation (an HC never sees another HC's `connected: true`/`false` — trivially true here since the lookup is always `current_tenant()`-scoped, but write the test anyway), unauthenticated → 401, client-role JWT → 403/404 per this codebase's convention.
...
**Re-send idempotency**: PHASE-04 already made Send idempotent (calling it twice re-finalizes and re-emails, by design). A raw token, once minted, cannot be recovered from its stored hash — so a second Send cannot resend the *same* link even if it wanted to. Resolution: **always mint a fresh token on Send**, every time, no reuse check. A second Send creates a new `LeadUploadToken` row and emails a new "Upload your results" link; the previous row is simply left unused and harmless — its `expires_at` is NULL until payment, so it was never in a live window to matter, and once a Lead has paid there's no further Send to trigger a second mint anyway (Send only fires from the HC review screen, before payment exists). Do not attempt to preserve the old link across a re-send — that isn't achievable without storing the raw token in plaintext somewhere, which is worse.
