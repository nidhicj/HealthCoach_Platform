...
| v1.5 | 2026-06-24 | SoJo + Claude | Cross-browser auth fix documented. Bug added to §4 (sign-in loop in Firefox/Safari due to `run.app` PSL cross-site boundary). Two lessons added to §7. Changelog updated. Fix: BFF proxy in `frontend/src/app/api/[...path]/route.ts`; `config.ts` `API_URL = ""`; `API_BASE_URL` secret → frontend URL; OAuth redirect URI updated. ADR-0005 and `diagrams/0001-system-architecture.md` amended same date. |
| v1.6 | 2026-06-25 | SoJo + Claude | `new URL()` silent-failure bug added to §4. Mock data migration to Supabase documented in §7 lessons. PHASE-09 marked production-verified: sign-in confirmed working in Chrome, mock data live in Supabase, dashboard loading. |
...
| Sign-in loop in Firefox and Safari (Chrome appeared to work) | `run.app` is in the Public Suffix List — frontend and backend are cross-site, not same-site. ADR-0005 §5 assumed same-eTLD+1 (Cloudflare Pages + Workers). Firefox Total Cookie Protection (dFPI) and Safari ITP block third-party cookies even with `SameSite=None; Secure`. The `refresh_token` cookie was set on the backend domain; the browser refused to send it on same-origin requests to the frontend. Chrome masked this because partitioned cookies are not yet on by default. | BFF proxy — added `frontend/src/app/api/[...path]/route.ts`: Next.js Route Handler that proxies all `/api/*` server-to-server to the backend. The OAuth callback (`302 + Set-Cookie`) is intercepted; `Set-Cookie` is re-emitted on the frontend domain. All browser requests are same-origin. `config.ts` `API_URL = ""`. `API_BASE_URL` Secret Manager value → frontend URL. Google Console OAuth redirect URI → frontend. ADR-0005 and system architecture diagram amended 2026-06-24. |
| All list endpoints silently returned empty data in production | `new URL(`${API_URL}/api/sessions`)` throws `TypeError: Invalid URL` in the browser when `API_URL = ""` — `new URL()` requires an absolute URL and does not accept relative paths. The error was caught by page-level error boundaries and rendered as empty state, never appearing in Cloud Run logs. Affected: `listSessions`, `listClients`, `listActionItems`, `listClientCheckIns`. | Replaced `new URL()` pattern with `URLSearchParams` across all four files (`sessions.ts`, `clients.ts`, `actionItems.ts`, `checkIns.ts`). Query string now built as `?${new URLSearchParams(...)}` and appended directly to the relative URL string. **Lesson**: `new URL()` is browser-safe only with an absolute URL. When `API_URL` is empty (same-origin proxy mode), always use `URLSearchParams` + string concatenation. Errors of this type only appear in the browser Console tab — Cloud Run logs are blind to them. Fixed 2026-06-25. |
