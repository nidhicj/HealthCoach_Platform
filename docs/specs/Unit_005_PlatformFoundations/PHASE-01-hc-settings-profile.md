...
**Goal:** Give the HC an editable business name (distinct from their personal Google identity) and a read-only view of their signed-in account, closing the smallest of the seven Platform Foundations gaps (`Unit_005_PlatformFoundations/SPEC-0001-platform-foundations.md`, Option C).
...
- **This branch is `feature/unit-004-one-stop-spot` — do not execute this plan here.** SoJo will stash this Unit_005 work onto a new branch first; only start Task 1 once that move has happened.
- No commits happen until SoJo has moved this work to its own branch — every "Commit" step below is written for that execution, not for the current session
...
```python
"""Integration tests for /api/settings/profile. Unit_005 PHASE-01."""
import pytest
...
**Spec coverage check** (against `Unit_005_PlatformFoundations/SPEC-0001-platform-foundations.md`, PHASE-01 section):
...
**Branch check**: this plan must not run on `feature/unit-004-one-stop-spot`. Execution starts only after SoJo moves this file (and the SPEC it implements) to a new branch.
