...
**Status**: Partially Implemented — see 2026-07-30 changelog entry for exact task-by-task state
**Date**: 2026-07-14
...
| 2026-07-30 | **Status corrected from "Proposed" to "Partially Implemented"** after this ADR was found missing entirely from `main` during a docs-currency audit, and its actual task-completion state was verified directly against code rather than assumed. Verified: **Task 1 (local dev DB rename) — done** (`chore(db): finish parivarthan_test -> tapas_test rename`, committed 2026-07-14). **Task 2 (backend package identity/JWT/UA) — done**, confirmed live in `backend/pyproject.toml` (`tapas-backend`), `backend/src/auth/jwt_utils.py` (`_ISSUER`/`_AUDIENCE` both `tapas.*`), `backend/src/lib/http.py` (UA string), `backend/src/main.py` (`title="Tapas API"`). **Task 3 (CI/deploy alignment) — NOT done**, and its own Step 1 hold is still unresolved: `.github/workflows/deploy.yml`, `docs/ops/deployment.md`, and `docs/ops/secrets-management.md` still reference `parivarthan-backend`/`parivarthan-api`, a service that was never real (confirmed live: only `hc-platform`/`hc-platform-backend` exist). This changelog entry's own docs-fix pass corrected the two `.md` files' prose (pure documentation, no execution risk) but deliberately did **not** touch `deploy.yml` itself — editing the actual CI workflow could trigger a real redeploy of the live service on the next push to `main`, exactly the risk this ADR's own Task 3 Step 1 flags, and that confirmation from SoJo still hasn't happened. **Task 4 (frontend cleanup) — done**, confirmed live in `frontend/tests/e2e/auth.spec.ts` (asserts `/tapas/i`) and no `parivarthan` residue in `frontend/package.json`/`tsconfig.json`. **Task 6 (final verification) — blocked on Task 3**; a repo-wide grep otherwise shows only the intentional historical mentions Task 5 Step 3 anticipated (session log, handover docs, this ADR itself), plus one previously-unflagged stray in `docs/specs/Unit_004_OneStopSpot/PHASE-01d-meeting-link.md` (fixed in this same pass — see that file). |
