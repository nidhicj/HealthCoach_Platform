...
| Database                         | **Supabase (free tier, `ap-south-1` Mumbai)** — managed Postgres; 500 MB DB, 50 K MAU; India region confirmed. Keep-alive via GitHub Actions cron ping daily (prevents free-tier pause; see `.github/workflows/supabase-keepalive.yml`). Upgrade path: Supabase Pro ($25/month) when 500 MB exceeded.                                                                                                                                                                                                            |
...
- Free-tier pause risk mitigated by a GitHub Actions keep-alive ping, daily (see `.github/workflows/supabase-keepalive.yml`). Data is not lost on pause — only accessibility is suspended.
...
| 2  | **Supabase free-tier project pauses** after 1 week of inactivity. <br />API becomes inaccessible until manually unpaused; data is preserved. | **Workaround**: GitHub Actions cron (`.github/workflows/supabase-keepalive.yml`) pings <br />the Supabase REST API daily — counts as activity, <br />prevents pause. If pause happens anyway: log in to Supabase <br />dashboard → unpause; takes ~1 min; data intact.                     |
...
| Date                 | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Reason                                                                                                                                                                                                                                                                                                        |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-08 (latest)  | **Supabase keep-alive job built.** Added `.github/workflows/supabase-keepalive.yml` — daily cron (`0 2 * * *`) plus `workflow_dispatch`, pings `{SUPABASE_URL}/rest/v1/` with the anon key, fails loudly on non-200. Cadence changed from the originally planned every-4-days to daily (still well within the 7-day pause threshold; negligible added cost). Removed the stale `docs/ops/supabase-keepalive.yml` follow-up entry and corrected the 3 references in this ADR that pointed to that nonexistent path — the job lives directly in `.github/workflows/` per the repo's folder map; no `docs/ops/` created. Confirmed no dead Cloudflare Workers workflow files remained in `.github/workflows/` (that cleanup had already happened in a prior session). | Backend hosting moved to Cloud Run in the 2026-06-19 entry below, which is when the original GitHub Actions setup was rebuilt — the Supabase keep-alive job was documented as a plan at that time but never actually committed. SoJo asked to close that gap and clean up any leftover CI references. |
| 2026-06-19            | **Backend hosting changed: Cloudflare Python Workers → GCP Cloud Run (`asia-south1`, Mumbai).** Cloudflare Python Workers eliminated — FastAPI is not supported (confirmed deployment error Jun 2026). Cloud Run adopted as primary: containerised FastAPI via Docker, free tier, India region, no platform-specific workarounds. **Database changed: AWS RDS Postgres Mumbai → Supabase (free tier, `ap-south-1` Mumbai).** Supabase free Mumbai region confirmed by SoJo. Free-tier pause mitigated by GitHub Actions keep-alive cron. DigitalOcean Bangalore remains as named fallback but now triggers against Cloud Run, not Workers. Workers-specific consequences (#1 beta instability, #3 async-only constraint, #4 subrequest limits, #5 User-Agent #68, #6 beta debugging time) removed — none apply to Cloud Run. New hosting-layer and DB-layer triggers added. Background jobs row updated: no longer a Workers workaround — standard external scheduler hitting Cloud Run endpoints. | Cloudflare Python Workers does not support FastAPI. AWS RDS dropped in favour of Supabase free tier — same India region, zero cost at MVP, zero provisioning overhead. |
