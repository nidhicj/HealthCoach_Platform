...
**What changed**: `API_BASE_URL` and `FRONTEND_URL` now point to `https://app.tapas.fitness`, replacing the raw `hc-platform-frontend-*.run.app` Cloud Run URL. See ADR-0008 for the full decision record (why a Cloudflare Worker reverse proxy, not Cloudflare's native Origin Rules [Enterprise-only] or Cloud Run's native Domain Mapping [unsupported in `asia-south1`]).
...
**Sections updated in this ADR**: §1 (redirect URI note now points at ADR-0008 for the domain layer in front of it).
...
| Date       | Change         | Reason                                            |
| ---------- | -------------- | ------------------------------------------------- |
| 2026-07-12 | Amendment: custom domain `app.tapas.fitness` replaces raw Cloud Run URL for `API_BASE_URL`/`FRONTEND_URL`. Section referenced: §1. New §Amendment block added. | ADR-0008 — Cloud Run's raw `.run.app` URL isn't a suitable public-facing domain; Cloudflare Worker now fronts it. |
